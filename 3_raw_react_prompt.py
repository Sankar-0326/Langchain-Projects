# add fre + inspect - we'll parse tool calls from raw text instead of structured json.
import re
import inspect
...
# CHANGE: Delete the JSON schema, Tools now live inside the prompt as plain text
...
def run_agent(question: str):
    ...
    for iteration in range(1, MAX_ITERATIONS+1):
        ...
        if not action_match or not action_input_match :
            print(
                "   [Parsing] Error: Could not parse Action or Action Input from LLM output."
            )
            break
        ...
        # CHANGE: History is one growing string re-sent every iteration (replaces message.append).
